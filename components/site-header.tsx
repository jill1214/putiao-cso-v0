      ...
      <div className="mx-auto flex h-[76px] max-w-[1240px] items-center justify-between px-5 sm:px-8">
        <Link href="/" className="group flex items-center gap-3" aria-label="Go to Putiao, My Place, My Home, CSO, Inc. homepage">
          ...
          <span className="flex min-w-0 flex-col leading-none">
            ...
            <span className="mt-1 max-w-[160px] text-[10px] font-medium leading-tight text-[#66736d]">Putiao, My Place, My Home,<br />CSO, Inc.</span>
          </span>
        </Link>
...
export function ArrowLink({ children, href = '#' }: { children: React.ReactNode; href?: string }) {
  return <HashLink href={href} className="group inline-flex items-center gap-2 text-sm font-extrabold text-[#0b6b3a]">{children}<ArrowUpRight className="size-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" /></HashLink>
}

export function Footer() {
  return <footer className="bg-[#10221b] text-white" id="contact"><div className="mx-auto max-w-[1240px] px-5 py-14 sm:px-8"><div className="grid gap-12 md:grid-cols-[1.3fr_1fr_1fr] md:gap-20"><div><div className="flex items-center gap-3"><LogoMark light /><div><p className="text-xs font-extrabold uppercase tracking-[0.18em]">PMPMH</p><p className="text-sm text-white/60">Putiao, My Place, My Home, CSO, Inc.</p></div></div><p className="mt-6 max-w-sm text-sm leading-7 text-white/55">People • Nature • Community • A brighter tomorrow. A community-led organization rooted in Putiao.</p></div><div><p className="mb-5 text-xs font-extrabold uppercase tracking-[0.18em] text-[#8dd86e]">Explore</p><div className="flex flex-col gap-3 text-sm text-white/65"><HashLink href="/#about" className="hover:text-white">About our story</HashLink><HashLink href="/#programs" className="hover:text-white">Our programs</HashLink><HashLink href="/#leadership" className="hover:text-white">Board of Directors</HashLink><HashLink href="/#updates" className="hover:text-white">Community updates</HashLink></div></div><div><p className="mb-5 text-xs font-extrabold uppercase tracking-[0.18em] text-[#8dd86e]">Find us</p><p className="text-sm leading-7 text-white/65">Putiao, Pilar 4714<br />Sorsogon, Philippines</p><p className="mt-4 text-xs italic text-white/40">Official contact details coming soon.</p></div></div><div className="mt-14 flex flex-col gap-3 border-t border-white/10 pt-6 text-xs text-white/40 sm:flex-row sm:items-center sm:justify-between"><span>© 2024 Putiao, My Place, My Home, CSO, Inc.</span><span>Community-led. Non-partisan. Non-political.</span></div></div></footer>
}

export function ButtonLink({ children, href = '/#contact-form', secondary = false }: { children: React.ReactNode; href?: string; secondary?: boolean }) {
  return <HashLink href={href} className={`inline-flex min-h-12 items-center justify-center rounded-full px-6 text-sm font-extrabold transition-all ${secondary ? 'border border-[#c9dbce] bg-white text-[#0b6b3a] hover:border-[#0b6b3a] hover:bg-[#eff8f2]' : 'bg-[#0b6b3a] text-white shadow-lg shadow-[#0b6b3a]/15 hover:bg-[#075530] hover:shadow-xl'}`}>{children}</HashLink>
}
...
export function HeroImage() { return <div className="relative min-h-[500px] overflow-hidden rounded-[30px] bg-[url('/putiao-river.png')] bg-cover bg-center shadow-2xl shadow-[#0b6b3a]/10"><div className="absolute inset-0 bg-gradient-to-t from-[#10221b]/80 via-[#10221b]/15 to-transparent" /><div className="absolute inset-x-0 top-0 h-28 bg-gradient-to-b from-[#10221b]/55 to-transparent" /><div className="absolute left-6 right-6 top-6 flex items-center justify-between"><span className="rounded-full border border-white/30 bg-[#10221b]/45 px-3 py-2 text-[10px] font-bold uppercase tracking-[0.16em] text-white backdrop-blur-sm">Putiao, Sorsogon</span><span className="grid size-10 place-items-center rounded-full border border-white/30 bg-[#10221b]/45 text-white backdrop-blur-sm"><Waves className="size-5" /></span></div><div className="absolute bottom-6 left-6 right-6"><p className="font-serif text-3xl italic tracking-tight text-white/90 sm:text-4xl">Our river.<br />Our home.<br />Our future.</p><div className="mt-6 flex items-center gap-3 text-xs font-medium text-white/75"><span className="h-px w-8 bg-[#8dd86e]" />A community worth discovering</div></div></div> }

export function FeatureBand() { return <div className="relative overflow-hidden rounded-[28px] bg-[#0b6b3a] px-6 py-12 text-white sm:px-12 md:py-16"><div className="absolute -right-10 -top-20 size-64 rounded-full border border-white/10" /><div className="absolute -bottom-28 right-20 size-72 rounded-full border border-white/10" /><div className="relative flex flex-col gap-8 md:flex-row md:items-end md:justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[0.2em] text-[#b4e69e]">Our place identity</p><h2 className="mt-4 max-w-2xl text-3xl font-extrabold tracking-[-0.04em] sm:text-5xl">Gateway between Albay and Sorsogon.</h2><p className="mt-4 max-w-xl text-sm leading-7 text-white/70">Putiao is not simply a place people pass through. It is a community worth discovering.</p></div><ButtonLink href="/discover-putiao" secondary>Discover Putiao</ButtonLink></div></div> }
...
export function ContactSection() { return <section className="scroll-mt-28 bg-[#eff8f2]" id="contact-form"><div className="mx-auto max-w-[1240px] px-5 py-20 sm:px-8"><div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-24"><div><SectionLabel>Get involved</SectionLabel><h2 className="text-4xl font-extrabold tracking-[-0.05em] text-[#10221b] sm:text-5xl">There is a place for you here.</h2><p className="mt-5 max-w-md text-sm leading-7 text-[#66736d]">Whether you want to volunteer, collaborate, share a story, or simply learn more, we would love to hear from you.</p><div className="mt-8"><CheckList items={['Volunteer in community activities','Collaborate on a local initiative','Share a Putiao story or tradition']} /></div></div><div className="rounded-[26px] border border-[#d1e5d6] bg-white p-6 sm:p-8"><ContactForm /></div></div></div></section> }
